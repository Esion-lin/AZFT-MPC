...
#include <map>
using namespace std;
void add_out(uint8_t* label,uint8_t &code, uint32_t &pos_itr, uint32_t &s_itr){
...
void to_char(std:string str, char* out_str){
	copy(str.begin(), str.end(), out_str);
}
...
bool read_model(std::string file_path){
    ...
    while(getline(file,strLine)){
        ...
        if(strLine.find("OUT") != strLine.npos){
        	...
        	add_out(str, code, pos_itr, s_itr);
        	continue;
    		...
    		//itrate structure
        	add_cov( w, l, h, label_in, label_out, kernel_size, kernel_w, kernel_l, kernel_h, is_pending, stride, ex_for_tee, w_itr, code, pos_itr, s_itr)
    		...
    		add_pooling(w, l, h, label_in, label_out, pooling_type, kernel_w, kernel_l, is_pending, stride, ex_for_tee, code, pos_itr, s_itr);
    		...
    		add_ReLU( w, l, h, label_in, label_out, alpha, ex_for_tee, code, pos_itr, s_itr);
    		...
    		add_BN( w, l, h, label_in, label_out, ex_for_tee, w_itr, code, pos_itr, s_itr);
...
bool read_code(std::string file_path){
    ...
    for(int i = 0; i < cmd.size(); i++){
    	if(!cmd[i].is_goto){
    		...
    		add_goto(cmd[i].target, dic_goto[cmd[i].output], code, pos_itr, s_itr);
    	}
    }
